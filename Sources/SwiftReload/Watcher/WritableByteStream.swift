...
public final class ThreadSafeOutputByteStream: WritableByteStream {
  private static let defaultQueue = DispatchQueue(
    label: "org.swift.swiftpm.basic.thread-safe-output-byte-stream")
  ...
  public init(_ stream: WritableByteStream) {
    self.stream = stream
    self.queue =
      (stream as? _WritableByteStreamBase)?.queue ?? ThreadSafeOutputByteStream.defaultQueue
  }
...
@available(*, deprecated, message: "use send(_:) function on WritableByteStream instead")
@discardableResult
public func <<< (stream: WritableByteStream, value: ByteStreamable & CustomStringConvertible)
  -> WritableByteStream
{
  value.write(to: stream)
  return stream
...
public struct Format {
  ...
  static public func asSeparatedList<T: ByteStreamable>(_ items: [T], separator: String)
    -> ByteStreamable
  {
  ...
  static public func asSeparatedList<T>(
    ...
    return TransformedSeparatedListStreamable(
      items: items, transform: transform, separator: separator)
  }
...
public var stdoutStream: ThreadSafeOutputByteStream = try! ThreadSafeOutputByteStream(
  LocalFileOutputByteStream(
    filePointer: stdout,
    closeOnDeinit: false))

/// Public stderr stream instance.
public var stderrStream: ThreadSafeOutputByteStream = try! ThreadSafeOutputByteStream(
  LocalFileOutputByteStream(
    filePointer: stderr,
    closeOnDeinit: false))