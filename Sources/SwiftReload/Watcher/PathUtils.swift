...
extension Path {
    ...
    private struct WindowsPath: Path, Sendable {
        ...
        init(string: String) {
            if string.first?.isASCII ?? false, string.first?.isLetter ?? false,
                string.first?.isLowercase ?? false,
    ...
    private struct UNIXPath: Path, Sendable {
        ...
        init(normalizingAbsolutePath path: String) {
            precondition(
                path.first == "/",
                "Failure normalizing \(path), absolute paths should start with '/'"
            )
            ...
            assert(result.count == capacity, "count: " + "\(result.count), cap: \(capacity)")
        ...
        func suffix(withDot: Bool) -> String? {
            ...
            // a suffix).
            let fIdx =
                string.index(bIdx, offsetBy: 1, limitedBy: string.endIndex) ?? string.startIndex
...
extension AbsolutePath {
    ...
    @available(
        *,
        deprecated,
        message: "use throwing `init(validating:relativeTo:)` variant instead"
    )
