...
public class FSWatch {
    ...
    public init(paths: [AbsolutePath], latency: Double = 0.2, block: @escaping EventReceivedBlock) {
        ...
        self.latency = latency
            ...
            self._watcher = NoOpWatcher(
                paths: paths,
                latency: latency,
                delegate: _WatcherDelegate(block: block)
            )
        #elseif os(Windows)
...
// MARK:- inotify
    ...
    public final class RDCWatcher {
        ...
        public func start() throws {
            ...
            self.watches.forEach { watch in
                watch.thread = Thread {
                    [delegate = self.delegate, queue = self.queue, weak watch] in
                            ...
                            while var pNotify = pNotify {
                                ...
                                let file: String =
                                    String(
                                        ...
                                        count: Int(pNotify.pointee.FileNameLength)
    ...
    public final class Inotify {
        ...
        public struct WatchOptions: OptionSet {
            ...
            public static let defaultFileWatchOptions: WatchOptions = [
                .deleteSelf, .moveSelf, .modify,
            ]
        ...
        private let callbacksQueue = DispatchQueue(
            label: "org.swift.swiftpm.\(Inotify.self).callback"
        )
        ...
        public init(
            paths: [AbsolutePath: WatchOptions],
            latency: Double,
            delegate: InotifyDelegate? = nil
        ) {
            self.paths = paths
            self.delegate = delegate
            self.settle = latency
